...
from kiro.config import PROXY_API_KEY, REGION, PROFILE_ARN
...
async def messages(
    ...
    # Per-request auth override from upstream proxy headers (kiro-go)
    req_refresh = request.headers.get("X-Kiro-Refresh-Token", "").strip()
    req_region = request.headers.get("X-Kiro-Region", "").strip() or REGION
    req_profile = request.headers.get("X-Kiro-Profile-Arn", "").strip() or PROFILE_ARN
    req_auth_method = request.headers.get("X-Kiro-Auth-Method", "").strip().lower()
    req_client_id = request.headers.get("X-Kiro-Client-Id", "").strip()
    req_client_secret = request.headers.get("X-Kiro-Client-Secret", "").strip()
    auth_pool = None
    if req_refresh:
        # 如果上游提供了 clientId/clientSecret（IdC/BuilderId），则强制走 AWS_SSO_OIDC 刷新链路
        force_oidc = req_auth_method in {"idc", "builderid", "enterprise"} or (req_client_id and req_client_secret)
        auth_manager: KiroAuthManager = KiroAuthManager(
            refresh_token=req_refresh,
            profile_arn=req_profile,
            region=req_region,
            client_id=req_client_id or None,
            client_secret=req_client_secret or None,
            creds_file=None,
            sqlite_db=None,
        )
        if force_oidc:
        ...
        # Pick account from auth pool (random + cooldown failover)
        auth_pool = getattr(request.app.state, "auth_pool", None)
        if auth_pool:
            auth_manager: KiroAuthManager = auth_pool.pick() or request.app.state.auth_manager
        else:
    ...
    if auth_pool:
        auth_pool.record_success(auth_manager)
    ...
    try:
        ...
        if response.status_code != 200:
            if auth_pool:
                auth_pool.record_error(auth_manager, response.status_code)
            try:
