...
from kiro.auth_pool import build_auth_pool
...
async def lifespan(app: FastAPI):
    ...
    # Optional multi-account auth pool from env KIRO_REFRESH_TOKENS (comma-separated)
    # When configured, requests can randomly select accounts and fail over on 402/429/5xx.
    refresh_tokens_env = os.getenv("KIRO_REFRESH_TOKENS", "").strip()
    auth_managers = [app.state.auth_manager]
    if refresh_tokens_env:
        extra_tokens = [t.strip() for t in refresh_tokens_env.split(",") if t.strip()]
    ...
    app.state.auth_pool = build_auth_pool(auth_managers)
    
    # Create model cache
    app.state.model_cache = ModelInfoCache()

    # Check if running in integrated mode (credentials from kiro-go headers)
    skip_startup_check = os.getenv("SKIP_STARTUP_CREDENTIAL_CHECK", "").lower() in ("true", "1", "yes")

    if skip_startup_check:
        # Integrated mode: Skip API call, use fallback models
        # Models will be loaded dynamically when requests come in with credentials
        logger.info("Integrated mode detected - using fallback models (credentials from request headers)")
        await app.state.model_cache.update(FALLBACK_MODELS)
        logger.debug(f"Loaded {len(FALLBACK_MODELS)} fallback models")
    else:
        # Standalone mode: Load models from Kiro API at startup
